...
### Overview
At my workplace we use Slack for communication and we used to have different types of Karma bots and apps. However, we eventually got rid of them. Instead of giving karma, we started giving each other different emojis like ![carrot](./readme_files/karrotawards_carrot.png), ![beer](./readme_files/karrotawards_beer.png) and ![potato](./readme_files/karrotawards_potato.png). One person would keep track of all awards that people got in a Word document. So I decided to create a Slack application specifically for purpose of awarding emojis and keeping track of them. I came up with the name __KarrotAwards__ - a combination of Karma + Carrot :laughing: - and this is how this fun application has started! :laughing:
...
- ```Help``` -> this will send private message to the User who called it. The message will explain all available functions and what they do. All User needs to do is to type __/karrotawards help__.
...
It is set up as [Slack](https://slack.com/) application. In the backend the data is stored in [MongoDB](https://www.mongodb.com/cloud/atlas); [uDrop](https://www.udrop.com/) service is used to host generated images. The code is setup to be deployed to [Heroku](https://www.heroku.com) platform. 

The application uses [Got](https://github.com/sindresorhus/got) and [Node HTML to Image](https://github.com/frinyvonnick/node-html-to-image) packages to make API requests and generate images from HTML.
...
### Setup and testing
First, Slack App needs to be configured and set up with __Slash Command__ endpoint:
...
After I released the application to my organization Slack, 25 users safely and happily switched to KarrotAwards and ditched their Word doc, moreover, 32 new Users started using the app in the first week of the release! :laughing: I even did a presentation about this project in a monthly digital Townhall!
...
### Future improvements
1) One improvement that I would really like to explore with this project is actual production use of [OpenAI API](https://openai.com/) and their GPT-3 engine to fully generate the award announcement messages on the fly. This could also be used for leaderboard announcements. For that I would need to experiment more with the API and GPT-3 engine to make sure it produces the results that are adequate for my use case. After that I would present my project to OpenAI team for approval.
2) Explore usability, availability and speed of some of the blockchain file hosting platforms, and integrate the use of those into this application. One of potential platforms could be [Sia](https://siasky.net/).